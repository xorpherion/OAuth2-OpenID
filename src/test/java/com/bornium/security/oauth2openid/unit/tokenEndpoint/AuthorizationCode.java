...
public class AuthorizationCode extends BaseTokenEndpointTests {
    ...
    public void inferiorScopeThanBefore() throws Exception {
        Common.testExchangeOn(server,
                () -> {
                    try {
                        return Common.preStepAndTokenRequest(getPreStep(), getGrantType(), getRedirectUri(), Constants.SCOPE_PROFILE, getClientId(), getClientSecret(), getUsername(), getPassword(), getDeviceCode());
                    } catch (Exception e) {
                        return Common.defaultExceptionHandling(e);
                    }
                },
